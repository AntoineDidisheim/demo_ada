...
df=df[['date','ticker','analyst','rec','ret']].sort_values(['ticker','analyst','date']).reset_index(drop=True)
df['y'] = df['rec']-df.groupby(['ticker','analyst'])['rec'].shift(1).dropna()
df = df.dropna()
df = df.merge(features)
df.to_pickle(data.pickle_data_dir+'df_peer.p')
